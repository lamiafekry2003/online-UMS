...
export default function Login() {
  ...
  return (
    ...
    <h1 className={` relative text-2xl  font-bold lef `}>User Management System</h1>
    ...
    <p className=' text-center text-sm mt-2 mb-6 text-neutral-500 font-normal leading-4'>Enter your credentials to access your account</p>
      <form action="" className=" flex flex-col mt-5" onSubmit={handleSubmit(onSubmit)}>
        <label htmlFor="username" className=' text-sm font-medium my-2 text-neutral-500'>User Name</label>
        <input type="text" placeholder="Enter your name" className={` border border-1 border-neutral-200 rounded-md p-2 text-text-neutral-500 outline-0 ${errors.username ?` focus:border-red-600 border-red-600`:` focus:border-amber-300`} `} {...register('username',{required:'username is Required!'})}/>
        {errors.username && <span className=' text-red-600'>{errors?.username?.message}</span>}
        {/* password */}
        <label htmlFor="pass"  className=' text-sm font-medium my-2 text-neutral-500'>Password</label>
        <input type="password" placeholder="Enter your password"  className={`border border-1 border-neutral-200 rounded-md p-2 text-text-neutral-500 outline-0 ${errors.password ?` focus:border-red-600 border-red-600`:` focus:border-amber-300`}`} {...register('password',{required:'Password is Required'})}/>
